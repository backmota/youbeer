...
<article id="post-<?php the_ID(); ?>" <?php post_class('article-image clearfix'); ?>>
    <header class="entry-header">
        <hgroup>
            <h3 class="entry-format image"><?php _e('Image', 'tfbasedetails'); ?></h3>
        </hgroup>
    </header><!-- .entry-header -->
    ...
    <?php else : ?>
        ...
        <h2 class="entry-title img-title"><a href="<?php the_permalink(); ?>" title="<?php printf(esc_attr__('Permalink to %s', 'tfbasedetails'), the_title_attribute('echo=0')); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
        <?php tfbasedetails_img_posted_on(); ?>
    <?php endif; ?>
