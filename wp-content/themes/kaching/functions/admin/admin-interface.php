...
$functions_path = TF_ADMINOPTIONS . '/';

function adminoptions_add_admin() {

    global $query_string;

    $themename = get_option('tf_themename');
    $shortname = get_option('tf_shortname');

    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'adminoptions') {
        if (isset($_REQUEST['tf_save']) && 'reset' == $_REQUEST['tf_save']) {
            $options = get_option('tf_template');
            tf_reset_options($options, 'adminoptions');
            header("Location: admin.php?page=adminoptions&reset=true");
            die;
        }
    }

    $tf_page = add_submenu_page('themes.php', 'Theme Admin Options', 'Theme Admin Options', 'edit_theme_options', 'adminoptions', 'adminoptions_options_page');

    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$tf_page", 'tf_load_only');
    add_action("admin_print_styles-$tf_page", 'tf_style_only');
}

add_action('admin_menu', 'adminoptions_add_admin');

/* ----------------------------------------------------------------------------------- */
/* Reset Function - tf_reset_options */
/* ----------------------------------------------------------------------------------- */

function tf_reset_options($options, $page = '') {
    ...
    //When Theme Options page is reset - Add the tf_options option
    if ($page == 'adminoptions') {
        $query_inner .= " OR option_name = 'tf_options'";
    }
...
/* ----------------------------------------------------------------------------------- */
/* Build the Theme Admin Options Page */
/* ----------------------------------------------------------------------------------- */
...
function adminoptions_options_page() {
    ...
    <div class="wrap" id="truethemes_container">
        ...
        <form action="" enctype="multipart/form-data" id="ofform">
            ...
            $return = adminoptions_machine($options);
            ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
    <?php echo $return[1] ?>
            ...
            <div class="save_bar_top">
                <img style="display:none;" src="<?php echo get_template_directory_uri() ?>/functions/admin/images/wpspin_light.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
        ...
        <form action="<?php echo esc_attr($_SERVER['REQUEST_URI']) ?>" method="post" style="display:inline" id="ofform-reset">
            <span class="submit-footer-reset">
                ...
                <input type="hidden" name="tf_save" value="reset" />
            </span>
        </form>
    </div>
...
/* ----------------------------------------------------------------------------------- */
/* Load required styles for Options Page - tf_style_only */
/* ----------------------------------------------------------------------------------- */

function tf_style_only() {
    wp_enqueue_style('admin-style', TF_FRAMEWORK . '/admin/admin-style.css');
    wp_enqueue_style('color-picker', TF_FRAMEWORK . '/admin/css/colorpicker.css');
    $color = get_user_option('admin_color');
    if ($color == "fresh") {
        wp_enqueue_style('admin-style-grey', TF_FRAMEWORK . '/admin/admin-style-grey.css');
    }
}

/* ----------------------------------------------------------------------------------- */
/* Load required javascripts for Options Page - tf_load_only */
/* ----------------------------------------------------------------------------------- */

function tf_load_only() {

    add_action('admin_head', 'tf_admin_head');

    wp_enqueue_script('jquery-ui-core');
    wp_register_script('jquery-input-mask', TF_FRAMEWORK . '/admin/js/jquery.maskedinput-1.2.2.js', array('jquery'));
    wp_enqueue_script('jquery-input-mask');
    wp_enqueue_script('color-picker', TF_FRAMEWORK . '/admin/js/colorpicker.js', array('jquery'));
    wp_enqueue_script('ajaxupload', TF_FRAMEWORK . '/admin/js/ajaxupload.js', array('jquery'));
          ...
          jQuery("a[title=submit]").click( function(){
            ...
            jQuery(this).parents("form").submit();
                ...
                jQuery('#ofform').submit(function(){
                    ...
                    var data = {
        <?php if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'adminoptions') { ?>
                                                                            type: 'options',
        <?php } ?>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - tf_ajax_callback */
/* ----------------------------------------------------------------------------------- */

add_action('wp_ajax_tf_ajax_post_action', 'tf_ajax_callback');

function tf_ajax_callback() {
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            update_option($id, esc_attr(stripslashes($new_value)));
                        }
                    }
...
function adminoptions_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                ...
                $output .= '<input class="ao-input" name="' . $value['id'] . '" id="' . $value['id'] . '" type="' . $value['type'] . '" value="' . $val . '" />';
                break;
            ...
            case 'select':
                ...
                $output .= '<select class="ao-input" name="' . $value['id'] . '" id="' . $value['id'] . '">';
            ...
            case "multicheck":
                ...
                foreach ($value['options'] as $key => $option) {
                    ...
                    $tf_key = $value['id'] . '_' . $key;
                    $saved_std = get_option($tf_key);
                    ...
                    $output .= '<input type="checkbox" class="checkbox of-input" name="' . $tf_key . '" id="' . $tf_key . '" value="true" ' . $checked . ' /><label for="' . $tf_key . '">' . $option . '</label><br />';
                }
                break;
            ...
            case "upload":
                ...
                $output .= adminoptions_uploader_function($value['id'], $value['std'], null);
            ...
            case "upload_min":
                ...
                $output .= adminoptions_uploader_function($value['id'], $value['std'], 'min');
            ...
            case "heading":
                ...
                $jquery_click_hook = "ao-option-" . $jquery_click_hook;
...
function adminoptions_uploader_function($id, $std, $mod) {
